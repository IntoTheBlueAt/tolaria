...
// Must match the __DEMO_VAULT_PATH__ that Vite compiles into the dev bundle,
// otherwise the vault switcher won't recognise this as "only the default vault".
const REMEMBERED_DEFAULT_VAULT_PATH = resolve(
  ...
  await page.addInitScript((config) => {
    ...
    Object.defineProperty(window, "__mockHandlers", {
      ...
      set(value) {
        ...
        const originalGetSettings = ref.get_settings as
        ...
        ref.check_vault_exists = (args: { path?: string }) =>
          args?.path === config.checkExistingPath;
      },
      get() {
        return ref;
      },
...
test("accepting telemetry consent on a fresh start opens the vault choice wizard @smoke", async ({
  page,
}) => {
...
test("telemetry consent still leaves the welcome wizard fully keyboard navigable @smoke", async ({
  page,
}) => {
  await mockFreshStart(page, {
    activeVault: null,
...
for (const action of ["accept", "decline"] as const) {
  test(`${action} telemetry still resumes onboarding with only a remembered default vault @smoke`, async ({
    page,
  }) => {
    await mockFreshStart(page, {
