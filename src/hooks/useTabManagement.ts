...
function requestNoteContent({
  path,
}: Pick<NoteContentCacheEntry, "path">): NoteContentCacheEntry {
...
async function loadNoteContent(
  ...
  return (
    prefetchCache.get(path)?.promise ?? requestNoteContent({ path }).promise
  );
}
...
function isAlreadyViewingPath(
  ...
  return (
    pathsMatch(activeTabPathRef.current, path) ||
...
function shouldApplyLoadedEntry(options: {
  ...
  return (
    cachedContent !== content || !pathsMatch(activeTabPathRef.current, path)
  );
}
...
function handleEntryLoadFailure(options: {
  ...
  if (isMissingNotePathError(error)) {
    ...
    Promise.resolve(onMissingNotePath?.(entry, error)).catch(
      (callbackError) => {
...
async function navigateToEntry(options: {
  ...
  if (
    !forceReload &&
    isAlreadyViewingPath(tabsRef, activeTabPathRef, entry.path)
  ) {
    syncActiveTabPath(activeTabPathRef, setActiveTabPath, entry.path);
    finishNoteOpenTrace(entry.path);
    return;
  ...
  try {
    ...
    const content = await loadNoteContent(
      entry.path,
      forceReload || cachedContent !== null,
    );
    markNoteOpenTrace(entry.path, "contentLoadEnd");
    if (
      !shouldApplyLoadedEntry({
        seq,
        navSeqRef,
        cachedContent,
        content,
...
export function useTabManagement(options: TabManagementOptions = {}) {
  ...
  const executeNavigationWithBoundary = useCallback(
    async (targetPath: string, navigate: () => void | Promise<void>) => {
      const seq = ++beforeNavigateSeqRef.current;
      const currentPath = activeTabPathRef.current;
      if (
        beforeNavigate &&
        currentPath &&
        !pathsMatch(currentPath, targetPath)
      ) {
        try {
  ...
  const handleSelectNote = useCallback(
    async (entry: VaultEntry) => {
      ...
      await executeNavigationWithBoundary(entry.path, () =>
        navigateToEntry({
          entry,
          navSeqRef,
          tabsRef,
  ...
  const openTabWithContent = useCallback(
    (entry: VaultEntry, content: string) => {
  ...
  const handleReplaceActiveTab = useCallback(
    async (entry: VaultEntry) => {
      if (!pathsMatch(entry.path, activeTabPathRef.current)) {
