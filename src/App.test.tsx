...
const AI_AGENTS_ONBOARDING_DISMISSED_KEY =
  "tolaria:ai-agents-onboarding-dismissed";
const CLAUDE_CODE_ONBOARDING_DISMISSED_KEY =
  "tolaria:claude-code-onboarding-dismissed";
...
describe("App", () => {
  ...
  it("shows keyboard shortcut hints", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Cmd\+P or Cmd\+O to search/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows onboarding after telemetry consent when no active vault is configured", async () => {
    ...
    mockCommandResults.check_vault_exists = (args?: { path?: string }) =>
      args?.path === "/Users/mock/Documents/Getting Started";
  ...
  it.each([
    ...
    async (buttonTestId) => {
      // Must match STATIC_DEFAULT_PATH (from __DEMO_VAULT_PATH__) so the vault
      // switcher recognises this as "only the default vault, not a user choice".
      const rememberedDefaultVaultPath =
        (typeof __DEMO_VAULT_PATH__ !== "undefined"
          ? __DEMO_VAULT_PATH__
          : "") || "/fallback-demo-vault";
  ...
  it("opens favorites directly into Neighborhood mode", async () => {
    ...
    fireEvent.click(
      within(sidebar.closest("div")?.parentElement as HTMLElement).getByText(
        "Alpha",
      ),
    );
