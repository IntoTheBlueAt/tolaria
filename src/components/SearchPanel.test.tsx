...
describe("SearchPanel", () => {
  ...
  it("renders search input when open", () => {
    ...
    expect(
      screen.getByPlaceholderText("Search in all notes..."),
    ).toBeInTheDocument();
  });
  ...
  it("shows empty state hint when no query", () => {
    ...
    expect(
      screen.getByText("Search across all note contents"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Enter to open · Esc to close"),
    ).toBeInTheDocument();
  });
  ...
  it("calls onClose when clicking overlay", () => {
    ...
    const overlay = screen
      .getByPlaceholderText("Search in all notes...")
      .closest(".fixed")!;
  ...
  it("performs keyword search", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("How to Design AI-first APIs"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("navigates results with arrow keys", async () => {
    ...
    await waitFor(() => {
      const resultTwo = screen
        .getByText("Refactoring Retreat")
  ...
  it("shows metadata subtitle with word count and links", async () => {
    ...
    await waitFor(() => {
      ...
      expect(bodyText).toMatch((1247).toLocaleString());
