...
vi.mock("../utils/url", async () => {
  const actual = (await vi.importActual(
    "../utils/url",
  )) as typeof import("../utils/url");
  return { ...actual, openExternalUrl: vi.fn().mockResolvedValue(undefined) };
});

const { openExternalUrl } =
  (await import("../utils/url")) as typeof import("../utils/url") & {
...
describe("NoteItem", () => {
  ...
  it("shows the title with filename metadata when a change status is present", () => {
    ...
    expect(screen.getByTestId("change-note-filename")).toHaveClass(
      "truncate",
      "text-[12px]",
      "leading-[1.5]",
  ...
  it("shows a neutral fallback when line stats are unavailable", () => {
    ...
    expect(screen.getByTestId("change-stat-fallback")).toHaveTextContent(
      "Diff unavailable",
    );
  });
  ...
  it("adds more breathing room between note sections", () => {
    ...
    expect(screen.getByTestId("note-content-stack").className).toContain(
      "space-y-2",
    );
  });
  ...
  it("colors relationship chips by target type and opens the related note on Cmd+click only", () => {
    ...
    expect(onClickNote).toHaveBeenCalledWith(
      linkedProject,
  ...
  it("preserves exact linked note title formatting in relationship chips", () => {
    ...
    expect(screen.getByTestId("property-chip-topics-0")).toHaveTextContent(
      "AI / ML",
    );
  });
  ...
  it("keeps explicit wikilink aliases in relationship chips", () => {
    ...
    const sourceEntry = makeEntry({
      ...
      isA: "Note",
      relationships: {
        "Belongs to": ["[[project/my-project|My Cool Project]]"],
      },
    });
    ...
    expect(screen.getByTestId("property-chip-belongs-to-0")).toHaveTextContent(
      "My Cool Project",
    );
  });
