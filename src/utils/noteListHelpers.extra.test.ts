...
describe("noteListHelpers extra coverage", () => {
  ...
  it("builds note subtitles for empty, linked, and edited notes", () => {
    ...
    expect(formatSubtitle(modifiedEntry)).toBe(
      `1h ago · ${(1200).toLocaleString()} words · 2 links`,
    );
    expect(formatSubtitle(emptyEntry)).toBe("Empty");
    expect(formatSearchSubtitle(modifiedEntry)).toBe(
      `1h ago · Created 2d ago · ${(1200).toLocaleString()} words · 2 links`,
    );
  });
  ...
  it("sorts entries by built-in and custom property comparators", () => {
    ...
    expect(
      [...entries]
        .sort(getSortComparator("title", "asc"))
        .map((entry) => entry.title),
    ).toEqual(["Alpha", "Beta", "Gamma"]);
    expect(
      [...entries]
        .sort(getSortComparator("created", "desc"))
        .map((entry) => entry.title),
    ).toEqual(["Alpha", "Beta", "Gamma"]);
    expect(
      [...entries]
        .sort(getSortComparator("status", "asc"))
        .map((entry) => entry.title),
    ).toEqual(["Alpha", "Gamma", "Beta"]);
    expect(
      [...entries]
        .sort(getSortComparator("property:Score", "asc"))
        .map((entry) => entry.title),
    ).toEqual(["Alpha", "Gamma", "Beta"]);
    expect(
      [...entries]
        .sort(getSortComparator("property:Start", "asc"))
        .map((entry) => entry.title),
    ).toEqual(["Alpha", "Gamma", "Beta"]);
    expect(
      [...entries]
        .sort(getSortComparator("property:Enabled", "asc"))
        .map((entry) => entry.title),
    ).toEqual(["Alpha", "Gamma", "Beta"]);
  });

  it("serializes, parses, loads, and saves sort preferences with migration support", () => {
    const serialized = serializeSortConfig({
      option: "property:Priority",
      direction: "desc",
    });
    expect(serialized).toBe("property:Priority:desc");
    expect(parseSortConfig(serialized)).toEqual({
      option: "property:Priority",
      direction: "desc",
    });
    ...
    localStorage.setItem(
      APP_STORAGE_KEYS.sortPreferences,
      JSON.stringify({
        __list__: "title",
    ...
    expect(localStorage.getItem(APP_STORAGE_KEYS.sortPreferences)).toBe(
      JSON.stringify({
        __list__: { option: "modified", direction: "desc" },
      }),
    );
    expect(
      localStorage.getItem(LEGACY_APP_STORAGE_KEYS.sortPreferences),
    ).toBeNull();
  ...
  it("filters view, folder, favorites, and pulse selections", () => {
    ...
    expect(
      filterEntries(
        entries,
        { kind: "view", filename: "work.view" },
        undefined,
        views,
      ).map((entry) => entry.title),
    ).toEqual(["Alpha"]);
    expect(
      filterEntries(entries, { kind: "folder", path: "projects" }).map(
        (entry) => entry.title,
      ),
    ).toEqual(["Beta"]);
    expect(
      filterEntries(entries, { kind: "filter", filter: "favorites" }).map(
        (entry) => entry.title,
      ),
    ).toEqual(["Alpha"]);
    ...
    expect(
      filterEntries(entries, allSelection).map((entry) => entry.title),
    ).toEqual(["Alpha", "Beta"]);
  });
  ...
  it("filters inbox entries by period and counts them", () => {
    ...
    expect(
      filterInboxEntries(entries, "week").map((entry) => entry.title),
    ).toEqual(["This Week"]);
    expect(
      filterInboxEntries(entries, "month").map((entry) => entry.title),
    ).toEqual(["This Week", "This Month"]);
    expect(
      filterInboxEntries(entries, "quarter").map((entry) => entry.title),
    ).toEqual(["This Week", "This Month", "This Quarter"]);
